...
namespace InventoryApp
{
    public partial class RevenueReportForm : Form
    {
        ...
        private void XuatBaoCaoBanHang()
        {
            ...
            // Header
            worksheet.Cells["B2"].Value = "BÁO CÁO BÁN HÀNG";
            worksheet.Cells["B5"].Value = $"Ngày bán: {DateTime.Now:dd/MM/yyyy}";
            worksheet.Cells["F2"].Value = "Từ ngày:";
            worksheet.Cells["G2"].Value = fromDate.ToString("dd/MM/yyyy");
            worksheet.Cells["F3"].Value = "Đến ngày:";
            worksheet.Cells["G3"].Value = toDate.ToString("dd/MM/yyyy");

            // Titles
            string[] headers = { "#", "Mã Hàng", "Tên Hàng", "Đơn vị tính", "Số lượng", "Đơn giá", "Thành tiền" };
            for (int i = 0; i < headers.Length; i++)
            {
            ...
            foreach (var item in items)
            {
                ...
                worksheet.Cells[row, 3].Value = item.TenHang;
                worksheet.Cells[row, 4].Value = item.DonViTinh;
            ...
            // Tổng tiền
            worksheet.Cells[row, 6].Value = "Tổng tiền:";
            worksheet.Cells[row, 7].Formula = $"SUM(G7:G{row - 1})";
